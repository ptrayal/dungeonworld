...
void oedit( CHAR_DATA *ch, char *argument )
{
	...
	if(!pArea) 
	{
		log_string("OEdit: unable to ascertain object's area! (Corruption/Broken Area)");
		send_to_char("An error has occured in OEDIT, leaving editor.\n\r",ch);
		edit_done(ch);
		return;
	}
...
void do_aedit( CHAR_DATA *ch, char *argument )
{
	...
	{
		send_to_char( "AEdit : You do not have the security to create a new area.\n\r", ch );
		return;
	}
	...
	if (!IS_BUILDER(ch,pArea))
	{
	send_to_char("You do not have access to that area.\n\r",ch);
	return;
	}
...
void do_redit( CHAR_DATA *ch, char *argument )
{
	...
	if ( !pRoom )
	{
		send_to_char( "REdit : That vnum does not yet exist.\n\r", ch );
		return;
	}

	if ( !IS_BUILDER(ch, pRoom->area) )
	{
		send_to_char( "REdit : You do not have access to this area.\n\r", ch );
		return;
	}
...
void do_oedit( CHAR_DATA *ch, char *argument )
{
	...
	{
		send_to_char( "You do not have access to this area.\n\r" , ch );
			return;
	}
...
void do_alist( CHAR_DATA *ch, char *argument )
{
	...
	for ( pArea = area_first; pArea; pArea = pArea->next )
	{
		add_buf(output, Format("%3d | %-27.27s | %-5d-%5d | %-12.12s | %d | %-10.10s\n\r",
		 pArea->vnum,
		 pArea->name,
		 pArea->min_vnum,
		 pArea->max_vnum,
...
void do_freevnum(CHAR_DATA *ch, char *argument)
{
    ...
    while (1) 
    {
        ...
        for (pArea = area_first; pArea; pArea = pArea->next) 
        {
            ...
            else if (pArea->min_vnum > vnum_min && (vnum_next == -1 
                || pArea->min_vnum < vnum_next))
