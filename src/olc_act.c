...
REDIT( redit_mshow )
{
	...
	{
	   send_to_char( "REdit: That is an invalid mob vnum.\n\r", ch);
	   return FALSE;
	}
...
REDIT( redit_oshow )
{
	...
	if ( !is_number( argument ) )
	{
	   send_to_char( "REdit: That is in invalid object vnum.\n\r", ch);
	   return FALSE;
	}
...
bool change_exit( CHAR_DATA *ch, char *argument, int door )
{
	...
	{
		send_to_char( "REdit:  Your key's item type must be KEY, for obvious reasons.\n\r", ch );
		return FALSE;
	}
...
OEDIT( oedit_addapply )
{
	...
	if ( IS_NULLSTR(type) || ( typ = flag_value( apply_types, type ) ) == NO_FLAG )
	{
	...
	if ( IS_NULLSTR(loc) || ( value = flag_value( apply_flags, loc ) ) == NO_FLAG )
	{
	...
	if ( IS_NULLSTR(bvector) || ( bv = flag_value( bitvector_type[typ].table, bvector ) ) == NO_FLAG )
	{
...
MEDIT( medit_damtype )
{
	...
	send_to_char( "For a list of damage types, type '? weapon'.\n\r", ch );
	return FALSE;
	}
...
MEDIT( medit_ac )
{
	MOB_INDEX_DATA *pMob;
	char arg[MAX_INPUT_LENGTH];
	int pierce = 0, bash = 0, slash = 0, exotic = 0;

	do   /* So that I can use break and send the syntax in one place */
	{
	if ( IS_NULLSTR(argument) )  break;
...
MEDIT ( medit_delmprog )
{
	...
	if (!is_number( mprog ) || IS_NULLSTR(mprog) )
	{
