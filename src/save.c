...
void fwrite_char( CHAR_DATA *ch, FILE *fp )
{
	...
	WriteNumber( fp, "Pos", ch->position == POS_FIGHTING ? POS_STANDING : ch->position );
	if (ch->practice != 0)
		WriteNumber( fp, "Prac", ch->practice );
	if (ch->train != 0)
		WriteNumber( fp, "Trai", ch->train );
	if (ch->saving_throw != 0)
		WriteNumber( fp, "Save",	ch->saving_throw);
	WriteNumber( fp, "Alig", ch->alignment );
	if (ch->hitroll != 0)
		WriteNumber( fp, "Hit", ch->hitroll );
	if (ch->damroll != 0)
		WriteNumber( fp, "Dam", ch->damroll );
	fprintf( fp, "ACs %d %d %d %d\n",	
		ch->armor[0],ch->armor[1],ch->armor[2],ch->armor[3]);
	if (ch->wimpy !=0 )
		WriteNumber( fp, "Wimp", ch->wimpy );
	...
	else
	{
		...
		WriteNumber( fp, "LLev", ch->pcdata->last_level );
