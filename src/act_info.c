...
void show_char_to_char_0( CHAR_DATA *victim, CHAR_DATA *ch )
{
    ...
    if ( IS_EVIL(victim) && IS_AFFECTED(ch, AFF_DETECT_EVIL) ) strcat( buf, "\tW(\tRRed Aura\tW)\tn " );
    if ( IS_GOOD(victim) && IS_AFFECTED(ch, AFF_DETECT_GOOD) ) strcat( buf, "\tW(\tBBlue Aura\tW)\tn " );
    ...
    if (IS_SET(victim->act,ACT_TRAIN))
    	strcat(buf, "[\tGTrainer\tn] ");
    if (IS_SET(victim->act,ACT_PRACTICE))
    	strcat(buf, "[\tGSkills\tn] ");
...
void do_scroll(CHAR_DATA *ch, char *argument)
{
	...
	if (IS_NULLSTR(arg))
	{
		...
		{
			send_to_char( Format("You currently display %d lines per page.\n\r", ch->lines + 2),ch);
		}
		return;
	}
	...
	send_to_char( Format("Scroll set to %d lines.\n\r",lines),ch);
	ch->lines = lines - 2;
}
...
void do_score( CHAR_DATA *ch, char *argument )
{
	...
	if ( IS_IMMORTAL(ch))
	{
		...
		if (ch->invis_level)
		{
			send_to_char( Format("  Invisible: level %d",ch->invis_level),ch);
		}

		if (ch->incog_level)
		{
			send_to_char( Format("  Incognito: level %d",ch->incog_level),ch);
		}
		send_to_char("\n\r",ch);
	}
...
void do_who(CHAR_DATA *ch, char *argument)
{
    ...
    bool rgfRace[MAX_PC_RACE] = {FALSE};
    ...
    for (int iRace = 0; iRace < MAX_PC_RACE; iRace++)
        rgfRace[iRace] = FALSE;
    for (int iClan = 0; iClan < MAX_CLAN; iClan++)
        rgfClan[iClan] = FALSE;
    ...
    while (!IS_NULLSTR(argument))
    {
        ...
        else
        {
            ...
            else
            {
                ...
                else if (iRace >= 0 && iRace < MAX_PC_RACE)
                {
                    fRaceRestrict = TRUE;
                    rgfRace[iRace] = TRUE;
                }
    ...
    // Iterate through connected characters.
    for (d = descriptor_list; d != NULL; d = d->next)
    {
        CHAR_DATA *wch;
        ...
        const char *iclass = class_table[wch->iclass].who_name;
        // Handle special cases for class names.
        if (wch->level >= MAX_LEVEL - 8 && wch->level <= MAX_LEVEL)
        {
            static const char *level_names[] = {"IMP", "CRE", "SUP", "DEI", "GOD", "IMM", "DEM", "ANG", "AVA"};
            iclass = level_names[MAX_LEVEL - wch->level];
        }
...
void do_wimpy( CHAR_DATA *ch, char *argument )
{
	...
	{
		send_to_char( "\tW[\tRWimpy\tW]\tn Wimpy set too high.\n\r", ch );
		return;
	}
